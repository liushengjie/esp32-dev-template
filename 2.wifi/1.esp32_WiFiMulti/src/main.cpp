...
using namespace std;
// 测试HTTPS通讯的网站
const char *host = "devapi.qweather.com/v7/weather/now?location=101210111&key=382d551435bb42f98bad763bcc00cf62";
...
void fetchBuffer()
{
  if ((wifiMulti.run() == WL_CONNECTED))
  {
    ...
    if (https.begin(httpsClient, "https://devapi.qweather.com/v7/weather/24h?location=101210111&key=382d551435bb42f98bad763bcc00cf62"))
    {
      ...
      if (httpCode > 0)
      {
        ...
        if (httpCode == HTTP_CODE_OK)
        {
          ...
          while (https.connected() && (len > 0 || len == -1))
          {
            ...
            size_t size = httpsClient.available();

            if (size)
            {
              // read up to 128 byte
              int c = httpsClient.readBytes(buff, ((size > sizeof(buff)) ? sizeof(buff) : size));
...
void setup()
{
  ...
  WiFi.mode(WIFI_STA);
  wifiMulti.addAP("owlj-iPhone 13 Pro", "123456789");
}
